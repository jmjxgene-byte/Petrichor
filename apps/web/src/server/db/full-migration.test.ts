...
describe("buildInitialMigrationSql", () => {
    ...
    it("包含当前业务基础表，且不再创建 Todo、普通聊天、便签和向量表", () => {
        ...
        expect(sql).toContain("display_name text not null default '站点维护者'")
        expect(sql).toContain("branding_json text not null default '{}'")
        expect(sql).not.toContain("mailto:zang@linux.do")
