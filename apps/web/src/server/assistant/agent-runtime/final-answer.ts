...
- Wiki 页面证据也必须保留 [n] 数字来源角标，Wiki 链接绝不能替代来源角标。
- 【Wiki 实体标注】正文提到检索结果中已有的 Wiki 实体/概念时，用 [[pageKey|原文中的词语]] 直接包住首次出现的原词；不要在句末额外追加页面标题。pageKey 必须来自检索结果，严禁编造。
...
- 直接回答用户的问题，不要复述执行过程，不要输出内部推理。
- 结论必须来自下面列出的证据。
- 【引用格式（最重要）】凡是来自 Wiki 页面的信息，必须用 [[pageKey|原文中的词语]] 包住正文里实际出现的概念，例如：[[concept-rag|RAG]] 是一种检索增强生成方法。不要写成“RAG 是一种方法 [[concept-rag|RAG]]”这种句末重复标题。pageKey 见每条证据的「Wiki 引用」提示。
