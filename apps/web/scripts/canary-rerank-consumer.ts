...
export function parseRerankResponse(raw: unknown, expectedCount: number) {
    ...
    if (parsed.results.length !== expectedCount || new Set(parsed.results.map(row => row.index)).size !== expectedCount || parsed.results.some((row, index) => row.index < 0 || row.index >= expectedCount
