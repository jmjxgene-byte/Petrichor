...
async function ssh(command: string, input?: string) {
    ...
    try {
        ...
        if (code !== 0 || Buffer.byteLength(output) > 64 * 1024) throw new Error("ssh_transfer_failed")
...
try {
    const build = await Bun.build({ entrypoints: [path.join(import.meta.dir, "remote-spool-synthetic-agent.ts")], target: "node", minify: true })
