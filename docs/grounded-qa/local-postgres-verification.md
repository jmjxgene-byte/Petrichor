...
**仍开放的问题：** 首轮升级/回滚通过后，Deep首个并发创建发生一次错误，当时报告仅截断了Drizzle SQL包装错误，无法确认SQLSTATE。随后补充安全SQLSTATE/字段/约束分类，普通复验47项通过，12组三并发补强48项通过，但未重现首轮错误。未修改应用逻辑，也不能宣称偶发错误已修复；不得据此直接通过生产发布门。测试限定固定旧基线和合成数据，不覆盖线上漂移、全部业务表内容或完整Supabase托管行为。
