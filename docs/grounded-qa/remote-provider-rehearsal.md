...
此设计尚未实现或演练，不是生产接线完成。下一步需要允许在运行时临时目录及宿主root-only目录创建**仅合成产物和测试工具**，用假provider测试逐项交接、ACK丢失、传输恢复及清理；真实凭证/数据库/模型均不访问，不改变现有服务配置、镜像、挂载、端口或flags。通过后再单独审批真实配置核验及新canary。
